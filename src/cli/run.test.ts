...
describe('runApply', () => {
  it('returns 0 and creates resources on a fresh manifest', async () => {
    ...
    const code = await runApply({ kind: 'apply', file: 'm.yaml', dryRun: false, watch: false, quiet: false, validateOnly: false }, deps);
    expect(code).toBe(0);
  ...
  it('returns 0 and makes no provider calls in dry-run', async () => {
    ...
    const code = await runApply({ kind: 'apply', file: 'm.yaml', dryRun: true, watch: false, quiet: false, validateOnly: false }, deps);
    expect(code).toBe(0);
    const eventsAfter =
      deps.r2.events.length + deps.worker.events.length + deps.kv.events.length;
...
describe('runDiff', () => {
  it('prints the plan but does not execute any mutations', async () => {
    ...
    const code = await runDiff(
      { kind: 'diff', file: 'm.yaml', output: 'text', verbose: false },
      deps,
    );
    expect(code).toBe(0);
